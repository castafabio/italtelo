class UpdateItalteloTable < ApplicationJob
  ...
  def send_to_gest!(resource, line_item, reference, skip, duration, inks, customer_machine, old_customer_machine)
    ...
    if skip
      tsql = "UPDATE avlav SET lce_qtaes = #{line_item.quantity}, lce_flevas = 'S', lce_stop = #{resource.ends_at.utc.to_s(:db)}, lce_tempese = #{duration}, lce_ultagg = #{DateTime.now}, lce_ink = #{inks}, lce_note = #{old_customer_machine}, lce_codcent = #{customer_machine.bus240_machine_reference}, lce_descent = #{customer_machine.name}, lce_codcope = #{line_item.italtelo_user.code}, lce_descope = #{line_item.italtelo_user.description} WHERE id = #{line_item.send(reference)}"
    else
      tsql = "UPDATE avlav SET lce_qtaes = #{line_item.quantity}, lce_start = #{resource.starts_at.utc.to_s(:db)}, lce_stato = 'C', lce_stop = #{resource.ends_at.utc.to_s(:db)}, lce_tempese = #{duration}, lce_ultagg = #{DateTime.now}, lce_ink = #{inks}, lce_note = #{old_customer_machine}, lce_codcent = #{customer_machine.bus240_machine_reference}, lce_descent = #{customer_machine.name} WHERE id = #{line_item.send(reference)}"
    end
