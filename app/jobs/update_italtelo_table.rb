class UpdateItalteloTable < ApplicationJob
  ...
  def send_to_gest!(resource, line_item, reference, skip, duration, inks, customer_machine, old_customer_machine)
    ...
    if skip
      ...
      tsql = "UPDATE avlav SET lce_qtaes = #{line_item.quantity}, lce_start = '#{resource.starts_at.strftime('%Y-%m-%dT%H:%M:%S%:z')}', lce_stato = 'C', lce_stop = '#{resource.ends_at.strftime('%Y-%m-%dT%H:%M:%S%:z')}', lce_tempese = #{duration}, lce_ultagg = '#{DateTime.now}', lce_ink = '#{inks}', lce_note = '#{old_customer_machine}', lce_codcent = '#{customer_machine.bus240_machine_reference}', lce_descent = '#{customer_machine.name}' WHERE id = #{line_item.send(reference)}"
    end
